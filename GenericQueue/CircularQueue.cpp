...
/*--------------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------------*/
BOOL createQueue(Queue * qp, int size, size_t dSize)
{
    if (qp == NULL) {  /* qp 포인터 NULL check*/
        return FALSE;
    }
    qp->queue = calloc(size, dSize);    /* queue 메모리 할당 */
...
/*--------------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------------*/
BOOL enqueue(Queue * qp, void* enqueData,  size_t dSize, void(*memCpy)(void *, void *))
{
    if (qp == NULL) {  /* qp포인터 NULL check*/
        return FALSE;
    }
...
//    qp->queue[qp->rear] = enqueData;
    memCpy((char *)qp->queue + qp->rear *dSize, enqueData);
...
/*--------------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------------*/
BOOL dequeue(Queue * qp, void* dequeData,  size_t dSize, void(*memCpy)(void *, void *),
             void(*dataFree)(void *))
{
    if (qp == NULL) {  /* qp포인터 NULL check*/
        return FALSE;
...
//    *dequeData = qp->queue[qp->front];
    memCpy(dequeData, (char *)qp->queue + qp->front * dSize);
    if (dataFree != NULL){
        dataFree((char *)qp->queue + qp->front *dSize);
    }
...
/*--------------------------------------------------------------------------------------
 ...
 Returns            : 없음
 --------------------------------------------------------------------------------------*/
void printQueue(const Queue * qp, size_t dSize, void(*dataPrint)(void *))
{
    if (qp == NULL) {  /* qp포인터 NULL check*/
        return;
    }
    ...
    {
        dataPrint((char *)qp->queue + i * dSize);
//        printf("%5d\n", qp->queue[i]);
    }
    printf("\n");
}
/*--------------------------------------------------------------------------------------
 ...
 Returns            : 없음
 --------------------------------------------------------------------------------------*/
void destroyQueue(Queue * qp, size_t dSize, void(*dataFree)(void *))
{
    ...
    for (int i = qp->front; i != qp->rear; i = (i + 1) % qp->size)
    {
        dataFree((char *)qp->queue + i * dSize);//부가메모리 해제
    }
